window.addEventListener('scroll', function () {
    ...
    parallax.style.setProperty('--scroll', scrollPosition * 0.5 + 'px');
});
...
function Particle() {
    ...
    this.size = 0;
    this.depth = 0;
    this.vx = 0; // Change from vy to vx
}

Particle.prototype = {
    constructor: Particle,
    update: function (width, height) {
        if (this.x > width) { // Change from this.y > height to this.x > width
            this.x = 1 - this.size; // Change from this.y = 1 - this.size to this.x = 1 - this.size
        }
        this.x += this.vx; // Change from this.y += this.vy to this.x += this.vx
    }
};
...
for (var i = 0; i < len; i++) {
    ...
    particle.size = ((particle.depth + 1) / 8) * 2.5; // Size of particles
    particle.vx = (particle.depth * .25) + 1 / Math.random();
    ...
    for (var i = 0, l = particles.length; i < l; i++) {
        ...
        for (var w = 0; w < particle.size; w++) {
            for (var h = 0; h < particle.size; h++) {
                ...
                idata.data[pData] = 255; // Red
                idata.data[pData + 1] = 165; // Green
                idata.data[pData + 2] = 0; // Blue
                idata.data[pData + 3] = 255; // Alpha
            }
        }
